...
export const ExposeStore = () => {
  ...
  // eval('const moduleRaid = new ModuleRaid()')
  ...
  window.Store.SendMessage = window.mR.findModule(
    (m) => m.addAndSendMsgToChat,
  )[0]
  window.Store.EditMessage = window.mR.findModule('addAndSendMessageEdit')[0]
  window.Store.SendSeen = window.mR.findModule('sendSeen')[0]
  window.Store.User = window.mR.findModule((m) => m.getMaybeMeUser)[0]
  ...
  window.Store.Validators = window.mR.findModule((m) => m.findLinks)[0]
  window.Store.VCard = window.mR.findModule('vcardFromContactModel')[0]

  window.Store.WidFactory = window.mR.findModule((m) => m.createWid)[0]
  ...
  window.Store.EphemeralFields = window.mR.findModule(
...
export const LoadUtils = () => {
  ...
  window.WWebJS.sendMessage = async (chat, content, options = {}) => {
    ...
    if (options.quotedMessageId) {
      const quotedMessage = window.Store.Msg.get(options.quotedMessageId)
  ...
  window.WWebJS.getChats = async () => {
    ...
    console.log('chat finalizado: ', chats)

    const chatPromises = chats.map(async (chat) =>
      window.WWebJS.getChatModel(chat),
    )
    console.log('chat Promises: ', chatPromises)

    return await Promise.all(chatPromises)
  }

  window.WWebJS.getContactModel = (contact) => {
    const res = contact.serialize()
    res.isBusiness =
  ...
  window.WWebJS.getProductMetadata = async (productId) => {
    ...
    const product = await window.Store.QueryProduct.queryProduct(
      sellerId,
      productId,
    )
